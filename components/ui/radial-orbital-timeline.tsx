...
export default function RadialOrbitalTimeline({
  ...
  const calculateNodePosition = (index: number, total: number, radius: number) => {
    ...
    const opacity = 0.5 + 0.5 * (Math.sin(radian) + 1) / 2;
  ...
  const orbitRadius = containerRef.current ? Math.min(containerRef.current.offsetWidth, containerRef.current.offsetHeight) * 0.35 : 200;
      ...
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          ...
          {timelineData.map((item, index) => {
            const position = calculateNodePosition(index, timelineData.length, orbitRadius);
            ...
            return (
              <div
                key={item.id}
                // FIX: Ref callbacks should not return a value.
                ref={(el) => {
